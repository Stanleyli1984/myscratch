    ...
    def recurse(self):
        ...
        for x in xrange(self.n):
            if x not in self.status:
                ...
                #        break
                #if can_place:
                has_conflict = lambda row: (finished_row - row) + self.status[row] == x or -(finished_row - row) + self.status[row] == x
                if not any (has_conflict(n) for n in xrange(finished_row)):
